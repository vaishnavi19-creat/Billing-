...
export class CShopController {
    ...
    static async getAllShops(request: express.Request, response: express.Response, next: express.NextFunction) {
        try {
            ...
            const limit = Array.isArray(request.query.limit) ? request.query.limit[0] : (request.query.limit as string) || '10';
            const pageNumber = Array.isArray(request.query.pageNumber) ? request.query.pageNumber[0] : (request.query.pageNumber as string) || '1';

            // Convert to numbers
            const limitNumber = parseInt(limit as string, 10);
            const pageNumberNumber = parseInt(pageNumber as string, 10);
            ...
            const arrObjShops: Array<getAllShops> = await objShopService.getAllShops(limitNumber, pageNumberNumber);
    ...
    static async filterShops(request: express.Request, response: express.Response, next: express.NextFunction) {
        try {
            ...
            const arrShopObjs: Array<getAllShops> = await objShopService.getAllShops(limitNumber, pageNumberNumber);
